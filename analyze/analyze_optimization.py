"""
Optimization Results Analyzer v2

Analyzes parquet files from VM optimization runs to find best parameter combinations
for live trading, grouped by IB parameters.
...
MAX_DRAWDOWN_FILTER = 10.0  # Max allowed drawdown in R
TOP_N = 5  # Number of top groups to show

# Data sources
DATA_SOURCES = {
    "GER40": [
        ANALYZE_DIR / "results_vm1_ger40" / "results_final.parquet",
        ANALYZE_DIR / "results_vm2_ger40" / "results_final.parquet",
    ],
    "XAUUSD": [
        ANALYZE_DIR / "results_vm3_xauusd" / "results_final.parquet",
        ANALYZE_DIR / "results_vm4_xauusd_partial" / "results_partial.parquet",
    ],
}

# Variation prefixes in parquet columns
VARIATIONS = ["ocae", "tcwe", "reverse", "rev_rb"]


def load_and_merge_data(symbol: str) -> pd.DataFrame:
    """Load and merge parquet files for a symbol."""
    print(f"[INFO] Loading {symbol} data...")
    dfs = []
    for path in DATA_SOURCES[symbol]:
        if path.exists():
...
def parse_params(df: pd.DataFrame) -> pd.DataFrame:
    ...
    params_list = df["params_json"].apply(json.loads).tolist()
...
def analyze_with_grouping(df: pd.DataFrame, group_key_func, mode_name: str) -> list:
    ...
    for group_key in groups:
        ...
        for var in VARIATIONS:
            ...
            if best:
                ...
                # Weighted Sharpe: sharpe * trades for each variation
                trades = best["trades"]
                if trades > 0:
        ...
        # Weighted average Sharpe = sum(sharpe_i * trades_i) / sum(trades_i)
        weighted_avg_sharpe = weighted_sharpe_sum / total_trades if total_trades > 0 else 0

        # Extract common params from first variation
    ...
    for i, g in enumerate(group_results[:TOP_N], 1):
        ...
        if "max_distance_pct" in str(group_key_func.__name__) and "maxdist" in str(group_key_func.__name__):
...
def analyze_symbol(symbol: str) -> dict:
    ...
    # Load data
    df = load_and_merge_data(symbol)
    df = parse_params(df)

    # Mode 1: IB + Buffer (same buffer for all variations)
    top_ib_buffer = analyze_with_grouping(df, get_group_key_ib_buffer, "IB + Buffer")

    # Mode 2: IB + Buffer + MaxDist (same buffer AND maxdist for all variations)
...
def create_excel_report(analysis: dict, output_path: Path):
    ...
    ws1.append([f"Optimization Analysis - {analysis['symbol']}"])
    ...
    ws3.append([f"Optimization Analysis - {analysis['symbol']}"])
...
def generate_params_dict(analysis: dict, mode: str = "ib_buffer") -> str:
    ...
    lines = [
        f"# {symbol} {version} parameters - VM Optimizer ({datetime.now().strftime('%Y-%m-%d')})",
...
def main():
    print("="*60)
    print("OPTIMIZATION RESULTS ANALYZER v2")
    ...
    for symbol in ["GER40", "XAUUSD"]:
        try:
            ...
            # Create Excel report
            excel_path = ANALYZE_DIR / f"optimization_analysis_{symbol}_v2.xlsx"
            ...
            params_path = ANALYZE_DIR / f"best_params_{symbol}_V7.py"
            ...
            params_path_strict = ANALYZE_DIR / f"best_params_{symbol}_V7_STRICT.py"
