"""
Single Group Backtest Worker - CONTROL PERIOD VERSION

Runs slow backtest for a single parameter group on control period data.
Returns metrics without generating trade charts.

Used by run_parallel_backtest_control.py as worker function.

Control Period: November 2025 - January 2026
"""
...
# Data paths - CONTROL PERIOD DATA
DATA_BASE_PATH = Path(__file__).parent.parent / "data"
DATA_PATHS_OPTIMIZED = {
    "GER40": DATA_BASE_PATH / "control" / "GER40_m1.parquet",
    "XAUUSD": DATA_BASE_PATH / "control" / "XAUUSD_m1.parquet",
}
...
def run_single_group_backtest(
    ...
    """
    Run backtest for a single parameter group on CONTROL period data.
    ...
    # Load data from CONTROL period parquet
    ...
    with open(summary_path, "w") as f:
        f.write(f"Backtest Results (CONTROL PERIOD): {group_id}\n")
        f.write("=" * 50 + "\n")
    ...
    if not skip_charts and trade_log:
        try:
            ...
            charts_count = generate_all_trade_charts(
                ...
                timezone=SYMBOL_TIMEZONES[symbol],
                ib_data_by_date=ib_data_by_date,
            )
            logger.info(f"Generated {charts_count} trade charts for {group_id}")
        except Exception as e:
            logger.warning(f"Failed to generate trade charts for {group_id}: {e}")
    ...
    if groups_file.exists():
        ...
        if groups:
            ...
            result = run_single_group_backtest(
                group=groups[0],
                start_date=datetime(2025, 11, 4, tzinfo=pytz.UTC),
                end_date=datetime(2026, 1, 20, tzinfo=pytz.UTC),
                output_dir=Path(__file__).parent / "output" / "test_control",
                skip_charts=False,
            )
