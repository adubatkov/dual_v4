"""
Parallel Slow Backtest Runner - CONTROL PERIOD VERSION

Runs multiple backtest groups in parallel using multiprocessing.
Designed for execution on VMs with 24 cores each.

Control Period: November 4, 2025 - January 20, 2026

Usage:
    python run_parallel_backtest_control.py --groups-file groups.json --workers 20
"""
...
def run_backtest_wrapper(args: tuple) -> Dict[str, Any]:
    ...
    try:
        ...
        from backtest.backtest_single_group_control import run_single_group_backtest
...
def run_parallel_backtests(
    ...
    logger.info(f"Starting parallel backtest (CONTROL PERIOD)")
...
def save_results_summary(results: List[Dict], output_dir: Path, symbol: str = None):
    ...
    with open(json_path, "w") as f:
        json.dump({
            ...
            "period": "CONTROL (Nov 2025 - Jan 2026)",
...
def main():
    parser = argparse.ArgumentParser(description="Run parallel slow backtests (CONTROL PERIOD)")
    ...
    parser.add_argument("--start-date", type=str, default="2025-11-04", help="Start date (YYYY-MM-DD)")
    parser.add_argument("--end-date", type=str, default="2026-01-20", help="End date (YYYY-MM-DD)")
    ...
    # Parse dates - CONTROL PERIOD defaults
    ...
    if args.output_dir:
        ...
        output_dir = Path(__file__).parent / "output" / f"parallel_control{symbol_suffix}"
